...
2. **Vishing** — caller impersonated a named employee, claimed to be locked out, and convinced the help desk to reset the target's Okta MFA enrollment. The call at MGM reportedly lasted under 10 minutes. No phishing link was ever clicked.
...
10. **Collection** — exfiltrated customer PII (loyalty program data, partial payment card data, driver license numbers, SSNs) — TBD — verify exact scope per victim; staged data in cloud storage prior to exfil. (T1530, T1074.002)
...
- Caesars Entertainment 8-K filing (September 2023) — confirms unauthorized actor obtained loyalty program member data; ransom-payment amount (~USD 15 M) reported by Wall Street Journal and Bloomberg. **TBD — verify** confirmed amount vs. public reporting.
